...
use fretwire_protocol::{
    Device, Support, DEVICES, PID_HELIX_FLOOR, PID_HX_STOMP, PID_HX_STOMP_XL,
};
...
#[test]
fn lookup_by_pid() {
    assert_eq!(Device::by_pid(PID_HX_STOMP).map(|d| d.name), Some("HX Stomp"));
    assert_eq!(Device::by_pid(PID_HELIX_FLOOR).map(|d| d.name), Some("Helix Floor"));
    assert_eq!(Device::by_pid(PID_HX_STOMP_XL).map(|d| d.name), Some("HX Stomp XL"));
...
fn lookup_by_preset_model_code() {
    ...
    assert_eq!(Device::by_model_code("P33").map(|d| d.pid), Some(PID_HX_STOMP));
    assert_eq!(Device::by_model_code("P21").map(|d| d.pid), Some(PID_HELIX_FLOOR));
...
fn verified_devices_are_fully_described() {
    for d in DEVICES.iter().filter(|d| d.support == Support::Verified) {
        assert!(d.model_code.is_some(), "{} is verified but has no model code", d.name);
        assert!(d.preset_device_id.is_some(), "{} is verified but has no device id", d.name);
        assert!(d.dsps.is_some(), "{} is verified but has no DSP count", d.name);
        assert!(d.snapshots.is_some(), "{} is verified but has no snapshot count", d.name);
    }
}
...
fn the_untested_device_claims_nothing_it_hasnt_shown_us() {
    ...
    assert_eq!(xl.dsp_count(), 1, "unknown DSP count falls back to one group");
}
...
fn verified_devices_come_first_so_open_prefers_them() {
    ...
    let first_untested =
        DEVICES.iter().position(|d| d.support == Support::Untested).unwrap_or(DEVICES.len());
    let last_verified =
        DEVICES.iter().rposition(|d| d.support == Support::Verified).unwrap_or(0);
    assert!(last_verified < first_untested, "verified devices must be listed before untested ones");
}
