//! The transport-neutral command surface — thin wrappers over `fretwire_core::Session`, exposing
//! the editor to any frontend transport. `fretwire-tauri` consumes it through one-line
//! `#[tauri::command]` wrappers; a future `fretwire-serve` (HTTP) is the second consumer this
//! crate exists for (see `docs/serve-mode.md`). The live session is held in [`AppState`] behind an
//! `Arc<Mutex<…>>`, so `connect` opens it once and every later command reuses it.
//!
//! **Threading:** all device I/O is blocking USB, so these commands are `async` and run the work on
//! a background thread via `spawn_blocking`. Running them synchronously would block the caller's
//! event loop (Tauri's webview thread, a server's reactor) — freezing the UI and deadlocking the
//! very call that's meant to return. Callers must therefore await these on a tokio runtime;
//! Tauri's `async_runtime` is one.
//!
//! **Events:** the two device-originated flows (the heartbeat, export progress) reach the frontend
//! through the [`events::EventSink`] each transport implements — see [`events`].
//!
...
/// `device-lost`.
pub fn spawn_heartbeat(sink: impl EventSink, session: Arc<Mutex<Option<Session>>>) {
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            if let Some(msg) = lost {
                sink.emit(Event::DeviceLost(msg));
            }
            let dtos = crate::dto::push_dtos(&pushes);
            if !dtos.is_empty() {
                sink.emit(Event::DevicePushes(dtos));
            }
        }
    });
}

/// The command result type: errors cross every transport as plain strings, exactly as Tauri
/// rejects an `invoke` — the frontend renders them with `String(e)` and nothing more.
...
pub async fn connect(state: &AppState) -> R<PresetDto> {
    let sess = state.session.clone();
    tokio::task::spawn_blocking(move || -> R<PresetDto> {
        let mut guard = sess
            .lock()
            .map_err(|_| "session lock poisoned".to_string())?;
        if guard.is_none() {
...
pub async fn disconnect(state: &AppState) -> R<()> {
    let sess = state.session.clone();
    tokio::task::spawn_blocking(move || {
        let taken = sess
            .lock()
...
pub async fn read_preset(state: &AppState) -> R<PresetDto> {
...
pub async fn undo(state: &AppState) -> R<PresetDto> {
    returning(state, |s| s.undo()).await
}

pub async fn redo(state: &AppState) -> R<PresetDto> {
    returning(state, |s| s.redo()).await
}

/// Jump the edit buffer to history timeline entry `index` (history-pane click / A-B compare).
pub async fn history_jump(state: &AppState, index: usize) -> R<PresetDto> {
...
pub async fn set_bypass(state: &AppState, slot: i64, bypassed: bool) -> R<PresetDto> {
    mutate_edit(
        state,
        move |s| {
            format!(
...
pub async fn set_param(state: &AppState, slot: i64, param_index: i64, value: f32) -> R<PresetDto> {
    mutate_edit(
        state,
...
pub async fn preview_param(state: &AppState, slot: i64, param_index: i64, value: f32) -> R<()> {
...
pub async fn add_block(state: &AppState, model_index: i64, paired_index: i64) -> R<PresetDto> {
    returning_edit(
        state,
...
pub async fn delete_block(state: &AppState, slot: i64) -> R<PresetDto> {
    returning_edit(
        state,
...
pub async fn clear_preset(state: &AppState) -> R<PresetDto> {
...
pub async fn revert_preset(state: &AppState) -> R<PresetDto> {
    returning_edit(
        state,
...
pub async fn reorder_block(state: &AppState, src_slot: i64, gap: usize) -> R<PresetDto> {
    returning_edit(
        state,
...
pub async fn move_before_split(state: &AppState, src_slot: i64) -> R<PresetDto> {
    returning_edit(
        state,
        move |s| format!("Move {}", s.slot_label(src_slot)),
...
pub async fn place_block(state: &AppState, src_slot: i64, dst_slot: i64) -> R<PresetDto> {
    returning_edit(
        state,
        move |s| format!("Move {}", s.slot_label(src_slot)),
...
pub async fn set_node_pos(state: &AppState, node: String, pos: i64, dsp: usize) -> R<PresetDto> {
...
pub async fn set_split_type(state: &AppState, split_slot: i64, model_index: i64) -> R<PresetDto> {
    returning_edit(
        state,
...
pub async fn assign_bypass(state: &AppState, slot: i64, switch: i64) -> R<PresetDto> {
    mutate_edit(
        state,
...
pub async fn unassign_bypass(state: &AppState, slot: i64, switch: i64) -> R<PresetDto> {
    mutate_edit(
        state,
...
pub async fn set_switch_color(state: &AppState, switch: i64, color: Option<i64>) -> R<PresetDto> {
    ...
    mutate_edit(
        ...
        move |s| {
            ...
            if source == 0 {
                ...
                format!(
                    "{} \u{2192} {p}",
                    crate::dto::source_name(source, s.assign_sources())
                )
            }
        },
...
pub async fn set_snapshot(state: &AppState, index: i64) -> R<PresetDto> {
    mutate(state, move |s| s.set_snapshot(index)).await
}

pub async fn goto_preset(state: &AppState, bank: i64, preset: i64) -> R<PresetDto> {
...
pub async fn save_preset(state: &AppState, bank: i64, slot: i64, name: String) -> R<PresetDto> {
    mutate(state, move |s| {
...
pub async fn rename_preset(state: &AppState, bank: i64, slot: i64, name: String) -> R<()> {
    run(state, move |s| {
...
pub async fn rename_snapshot(state: &AppState, index: i64, name: String) -> R<PresetDto> {
    mutate_edit(
        state,
...
pub async fn list_presets(state: &AppState, bank: Option<i64>) -> R<Vec<PresetListItem>> {
...
/// still shows none.
pub async fn setlists(state: &AppState) -> R<Vec<String>> {
    run(state, |s| {
        Ok(s.device()
            .setlist_names()
            .iter()
...
pub async fn cancel_export(state: &AppState) -> R<()> {
...
pub async fn copy_preset(state: &AppState) -> R<String> {
...
pub async fn paste_preset(state: &AppState) -> R<PresetDto> {
...
pub fn clipboard_preset(state: &AppState) -> Option<String> {
    state
        .clipboard
        .lock()
        .ok()
...
pub async fn copy_block(state: &AppState, slot: i64) -> R<String> {
...
pub async fn paste_block(state: &AppState, slot: i64) -> R<PresetDto> {
...
pub fn clipboard_block(state: &AppState) -> Option<String> {
    state
        .block_clipboard
        .lock()
        .ok()
...
pub async fn categories(state: &AppState) -> R<Vec<CategoryDto>> {
...
pub async fn device_numbering(state: &AppState) -> R<Option<String>> {
    run(state, |s| {
...
pub async fn settings_read(state: &AppState, all: bool) -> R<Vec<SettingDto>> {
...
pub async fn settings_write(state: &AppState, id: i64, value: f64) -> R<SettingDto> {
    use fretwire_core::fretwire_protocol::settings;
...
pub async fn ir_list(state: &AppState) -> R<Vec<IrSlotDto>> {
    run(state, |s| {
...
pub async fn ir_scan(state: &AppState) -> R<Vec<IrSlotDto>> {
    run(state, |s| {
...
pub async fn ir_delete(state: &AppState, slot: i64) -> R<Vec<IrSlotDto>> {
    run(state, move |s| {
        s.ir_delete(slot)?;
        Ok(s.ir_directory()?.iter().map(IrSlotDto::from).collect())
    })
...
pub async fn ir_rename(state: &AppState, slot: i64, name: String) -> R<Vec<IrSlotDto>> {
    run(state, move |s| {
