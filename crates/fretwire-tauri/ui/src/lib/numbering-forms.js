...
//
// **Polarity.** Setting 27 is a flag whose `on` label is `000-127` and whose `off` label is
// `01A-32D` (`fretwire-protocol/src/settings.rs`), so `true` means the *flat* form. The Rust side
// turns the same bool into the same two words in `commands.rs::numbering_word`. Getting this
