...
17 commits since [`v0.4.0`](https://github.com/john-baxter-dev/fretwire/releases/tag/v0.4.0).
...
- **`fretwire-serve` and `fretwire-mcp` are source builds in this release** — `cargo build -p
  fretwire-serve` after `npm run build` in `crates/fretwire-tauri/ui`, and `cargo build -p
  fretwire-mcp` — not release assets yet. Serve mode has no TLS of its own; put it behind a
  tunnel or a proxy beyond a trusted LAN.
...
Grab the `.deb`, `.rpm`, `.AppImage` or the Arch `.pkg.tar.zst` for the GUI, or a static CLI
tarball — `fretwire-cli-x86_64-linux-musl.tar.gz` or `fretwire-cli-aarch64-linux-musl.tar.gz` —
that runs on any distro. The deb, rpm and Arch package install the udev rule and the CLI alongside
the app; for the AppImage or the tarballs, install `packaging/70-hxstomp.rules` yourself and replug
the pedal.
